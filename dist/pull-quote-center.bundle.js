...
var PQC = function () {
  function PQC($this, options) {
    _classCallCheck(this, PQC);
  ...
  _createClass(PQC, [{
    ...
    value: function init() {
      ...
      $this.find(this.opts.pquote_selector).each(function (index) {
        ...
        $(rhc).perWordAction(function ($word, line) {
    ...
    key: 'step_fine_tuner',
    value: function step_fine_tuner(line, error_offset, $word, $clone, pquoteTop, text, prev_error_offset, counter, bestTracker) {
      var _this = this;
      counter++;
      var MAX = 10;
      if (counter > MAX) {
      ...
      if (error_offset >= error_limit) {
        ...
        if (this.opts.debug) {
          console.log('nextLineEl?', $nextLineEl_ideal.length, $nextLineEl_backup.length, $nextLineEl_worst_case.length);
        }
        ...
        if (this.opts.debug) {
          console.log('prevLineEl?', $prevLineEl_ideal.length, $prevLineEl_backup.length, $prevLineEl_worst_case.length);
        }
